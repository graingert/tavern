...
class BuildDocs(Command):
    ...
    def run(self):
        ...
        self.execute(
            check_call, [["make", "-C", "docs", "html"]], msg="Making html docs"
        )
...
class DeployPypi(Command):
    ...
    def finalize_options(self):
        if not self.repository:
            raise RuntimeError(
                "Need a repository to upload to! Run 'python setup.py upload_twine' for options."
            )
    ...
    def run(self):
        ...
        if len(os.listdir("dist")) > 1:
            raise RuntimeError(
                "More than one package in dist/ - only one can be present to upload! Delete the dist/ folder before running this command."
            )
        ...
        self.execute(check_call, [args], msg="Uploading package to pypi")


TESTS_REQUIRE = ["pytest-cov", "colorlog", "mock", "faker"]
